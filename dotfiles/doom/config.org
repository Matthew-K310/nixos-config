#+title: Matthew Kennedy's Doom Emacs Configuration
#+STARTUP: content
...
Set variable font sizes for org-mode headings for improved readability.
...
#+end_src
*** Colorscheme
Moved from Nord to Compline, as it is more minimal and less blue.
#+begin_src elisp
...
*** org
**** org-mode
Set some `org` defaults, along with some helpful bindings.
#+begin_src elisp
(require 'org)
...
Clock in on start, clock out when state changed to `DONE`.
#+begin_src emacs-lisp
...
A function to move tasks to `done.org`.
#+begin_src emacs-lisp
...
Install and setup org-modern for better document formatting.
#+begin_src emacs-lisp
...
Replace asterisk headings with "H1", "H2", etc.
#+begin_src emacs-lisp
...
**** org-capture
***** capture templates
Set up custom `org-capture` templates.
#+begin_src elisp
(after! org
...
***** better bookmark and contact organization
#+begin_src emacs-lisp
(after! org
...
#+end_src
**** emacs-everywhere
Use `thanos` for `emacs-everywhere`-like behavior on Wayland systems (specifically Hyprland).
#+begin_src elisp
...
**** Denote
***** base config
Set up Denote for Zettlekasten note-taking and for larger projects.
#+begin_src elisp
(use-package denote
  :ensure t
...
*** org-present
*** ledger-mode
For being able to add ledger src blocks in org-mode files.
#+begin_src emacs-lisp
(after! ledger-mode
