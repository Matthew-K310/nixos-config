...
(defvar my/font-profiles
  '(("Iosevka Nerd Font Mono" :var "Alegreya" :size 14 :var-size 16)
...
(defun my/select-font ()
  (interactive)
  (let* ((choice (completing-read "Font profile: "
                                  (mapcar #'car my/font-profiles)
                                  nil t))
         (profile (assoc choice my/font-profiles))
         (mono     (car profile))
         (var      (plist-get (cdr profile) :var))
         (size     (plist-get (cdr profile) :size))
         (var-size (plist-get (cdr profile) :var-size)))
    (setq doom-font              (font-spec :family mono :size size)
          doom-variable-pitch-font (font-spec :family var  :size var-size)
          doom-big-font          (font-spec :family mono :size (+ size 7)))
    (doom/reload-font)
    (message "Switched to %s (size %s)" mono size)))

(map! :leader :desc "Select font profile" "t F" #'my/select-font)
...
(defun my/eshell-float ()
  "Launch a temporary frame with a clean buffer for typing."
  (interactive)
  (require 'eshell)
  (let ((frame (make-frame '((name . "eshell-float")
                             (fullscreen . 0)
                             (undecorated . t)
                             (width . 90)
                             (height . 30)
                             (explicit-eshell . t))))
        (buf (get-buffer-create "eshell-float")))
    (select-frame frame)
...
(defun my/new-frame-with-eshell ()
  "Create a new frame and immediately open eshell in it."
  (interactive)
  (require 'eshell)
  (let ((new-frame (make-frame '((explicit-eshell . t)))))
    (select-frame new-frame)
    (delete-other-windows)
    ;; eshell creates and switches to the buffer, but returns nil
    (eshell (format "*eshell-%s*" (frame-parameter new-frame 'name)))
    (company-mode -1)
    ;; Now current-buffer is the eshell buffer
    (delete-other-windows)))
...
(after! org
  ...
  (setq org-capture-templates
        ...
        '(("t" "Todo" entry
          ...
          ;; s for save link
          ("s" "Save Link")
          ("sc" "Save Link (Clipboard)" entry (file+headline "~/org/bookmarks.org" "Bookmarks")
           "** %(org-web-tools-insert-link-for-clipboard-url)\n:PROPERTIES:\n:CAPTURED: %t\n:END:\n%?" :empty-lines 1 :prepend t)
          ("sp" "Save Link (Paste)" entry (file+headline "~/org/bookmarks.org" "Bookmarks")
           "** %(org-web-tools-insert-link-for-given-url)\n:PROPERTIES:\n:CAPTURED: %t\n:END:\n%?" :empty-lines 1 :prepend t)
          ("sw" "Save Link (Watch Later)" entry (file+headline "~/org/bookmarks.org" "Watch Later")
           "** %(org-web-tools-insert-link-for-given-url)\n:PROPERTIES:\n:CAPTURED: %t\n:END:\n%?" :empty-lines 1 :prepend t)
          )))
...
(defun my/copy-micro-to-site ()
  ...
  (interactive)
  (let ((src "~/org/micro.org")
        (dest "~/dev/projects/matthew-kennedy.com/site/content/micro.org"))
    (copy-file src dest t)
    (message "Copied micro.org to %s" dest)))
